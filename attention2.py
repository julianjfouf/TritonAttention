...
@triton.jit
def _causal_attention_kernel2_1(
    ...
    min_col = group_col * BLOCK_SIZE_T2
    max_row = group_row * BLOCK_SIZE_T1 + BLOCK_SIZE_T1 - 1
    if (min_col > max_row): return
...
@triton.jit
def _causal_attention_kernel2_2(
...
def causal_attention2(q, k, v):
    ...
    grid = lambda meta: (B * H, triton.cdiv(T, meta["BLOCK_SIZE_T1"]), triton.cdiv(T, meta["BLOCK_SIZE_T2"]))
    _causal_attention_kernel2_1[grid](
    ...
    _causal_attention_kernel2_2[grid](
        qk, qk.stride(0), qk.stride(1), qk.stride(2),
    ...
    return out.view(B, H, T, C), qk.view(B, H, T, T)
